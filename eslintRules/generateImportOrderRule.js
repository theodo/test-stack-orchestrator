...
const filterDirent = dirent =>
  dirent.isDirectory() || (dirent.name.includes('.ts') && !dirent.name.includes('.test.ts'));

const formatPathGroupsFromDirent = dirent => ({
  pattern: dirent.isDirectory() ? `${dirent.name}{,/**}` : dirent.name.replace('.ts', ''),
  group: 'internal',
});
...
const generateImportOrderRule = (dirname, tsConfigPath = 'tsconfig.json') => {
  ...
  const baseUrlPaths = readdirSync(path.resolve(dirname, tsConfig.compilerOptions.baseUrl), {
    withFileTypes: true,
  })
    .filter(filterDirent)
  ...
  return {
    'import/order': [
      'error',
      {
        pathGroups: [{ pattern: '@canopia/**', group: 'unknown' }, ...pathGroups],
        groups: [['external', 'builtin'], 'unknown', 'internal', ['parent', 'sibling', 'index']],
        alphabetize: {
          order: 'asc',
